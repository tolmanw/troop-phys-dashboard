...
for username, info in refresh_tokens.items():
    ...
    # --- Fetch last three months activities for athletes.json ---
    activities = []
    for start in month_starts:
        after_ts = int(start.timestamp())
        activities.extend(fetch_activities(access_token, after_ts))
    ...
    # --- Generate per-activity JSONs for Jan + next 6 months dynamically ---
    now_uk = uk_now()
    current_year = now_uk.year
    current_month = now_uk.month

    for m_offset in range(0, 6):  # Jan + next 6 months
        month_num = m_offset + 1  # 1=Jan, 2=Feb...
        month_start = datetime(current_year, month_num, 1, tzinfo=timezone.utc)
        after_ts = int(month_start.timestamp())
        days_in_m = days_in_month(month_start)
        month_abbr = month_start.strftime("%b")
        challenge_data = {act_type:{} for act_type in ALL_TYPES}

        month_activities = fetch_activities(access_token, after_ts)
        for act_type in ALL_TYPES:
            daily_array = [0.0] * days_in_m
            for act in month_activities:
                if act.get("type") != act_type:
                    continue
                dt = datetime.strptime(act["start_date_local"], "%Y-%m-%dT%H:%M:%S%z")
                if dt.year != month_start.year or dt.month != month_start.month:
                    continue
                idx = dt.day - 1
        ...
        with open(filename,"w") as f:
            json.dump({
                ...
                "month_names": [f"{month_start.strftime('%B %Y')}"],
